...
export default function PlaybooksPage() {
    ...
    return (
        <CoreWrapper>
             ...
             <div className="flex flex-col md:flex-row gap-4 mb-10">
                 ...
                 <div className="relative">
                     <button
                        ...
                        className="h-full px-4 rounded-xl bg-zinc-100/50 dark:bg-zinc-900/50 border border-black/5 dark:border-white/5 flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-300 hover:text-black dark:hover:text-white hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-all min-w-[140px] justify-between"
                     >
                         <span className="flex items-center gap-2">
                     ...
                     {showFilterMenu && (
                         <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-[#18181b] border border-black/10 dark:border-white/10 rounded-xl shadow-2xl p-1 z-50 animate-in fade-in zoom-in-95 duration-100">
                             {[
                                 ...
                                 >
                                     <DynamicIcon name={opt.iconName} className="w-3.5 h-3.5"/>
                                     {opt.label}
                                 </button>
                             ))}
                         </div>
                     ...
                     {filtered.map(doc => (
                         <div key={doc.id} className="relative group/card perspective-1000">
                             <div className={`
                                ...
                                ${viewMode === 'grid'
                                    ? 'bg-white/40 dark:bg-black/40 backdrop-blur-xl rounded-[2rem] h-full flex flex-col hover:translate-y-[-4px] hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]'
                                    : 'bg-white/40 dark:bg-black/40 backdrop-blur-xl rounded-2xl p-5 flex items-center justify-between hover:bg-black/5 dark:hover:bg-white/5'
                                }
                             `}>
                                 {/* Click Target */}
                                 ...
                                 <div className={`relative z-10 w-full ${viewMode === 'list' ? 'flex flex-row items-center justify-between p-6 gap-8' : 'flex flex-col h-full'}`}>
                                     ...
                                     {/* Grid View: Image Top */}
                                     {viewMode === 'grid' && (
                                         <div className="relative h-48 w-full bg-zinc-100 dark:bg-zinc-900 overflow-hidden border-b border-black/5 dark:border-white/5">
                                            {doc.coverImage ? (
                                                <Image
                                                    src={doc.coverImage}
                                                    alt={doc.title}
                                             ...
                                             <div className="absolute top-4 right-4 flex gap-2">
                                                 {doc.lockedBy && (
                                                     <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/60 dark:bg-black/60 backdrop-blur-md border border-red-500/20 text-[10px] uppercase tracking-wider font-bold text-red-500">
                                                 ...
                                                 <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border backdrop-blur-md text-[10px] uppercase tracking-wider font-bold ${
                                                     ...
                                                     doc.visibility === 'CORE' ? 'bg-white/60 dark:bg-black/60 border-red-500/20 text-red-400' :
                                                     'bg-white/60 dark:bg-black/60 border-black/10 dark:border-white/10 text-zinc-500 dark:text-zinc-400'
                                                 }`}>
                                         ...
                                         {viewMode === 'grid' ? (
                                             <>
                                                <div className="p-4 flex items-start justify-between gap-4 mb-2">
                                                    <h3 className="text-lg font-bold text-black dark:text-white line-clamp-2 leading-tight group-hover:text-red-400 transition-colors">
                                                        {doc.title}
                                                    </h3>
                                             ...
                                             <>
                                                 <div className="flex items-center justify-between mb-2">
                                                    <h3 className="font-bold text-black dark:text-white text-xl truncate group-hover:text-red-400 transition-all duration-300">
                                                        {doc.title}
                                                    </h3>
                                                    ...
                                                    <div className="flex items-center gap-2 ml-4 flex-shrink-0">
                                                         ...
                                                         <div className={`px-2 py-0.5 rounded-full border text-[10px] font-bold ${
                                                             doc.visibility === 'PUBLIC' ? 'bg-zinc-200/50 dark:bg-zinc-800/50 border-black/10 dark:border-white/10 text-zinc-600 dark:text-zinc-300' :
                                                 ...
                                                 <div className="flex-1">
                                                     <p className="text-zinc-500 dark:text-zinc-400 text-sm line-clamp-2 leading-relaxed mb-4">
                                                         {doc.description || "No description provided."}
                                                     </p>
                                                 </div>
                                    ...
                                    {viewMode === 'list' && doc.coverImage && (
                                        <div className="w-32 h-24 rounded-lg overflow-hidden border border-black/10 dark:border-white/10 flex-shrink-0 bg-zinc-100 dark:bg-zinc-900">
                                            <img 
                                               src={doc.coverImage} 
                                               alt={doc.title} 
                             ...
                             {canDelete && (
                                 <div className={`absolute z-30 ${viewMode === 'grid' ? 'top-4 right-4' : 'top-1/2 -translate-y-1/2 right-4'}`}>
                                     <button 
                                         ...
                                         className="p-2 rounded-full hover:bg-black/10 dark:hover:bg-white/10 text-zinc-500 hover:text-black dark:hover:text-white transition-colors opacity-0 group-hover/card:opacity-100 bg-white/50 dark:bg-[#121212]/50 backdrop-blur-sm border border-black/5 dark:border-white/5"
                                     >
                                         <MoreHorizontal className="w-5 h-5"/>
                                     </button>
 
                                     {activeMenuId === doc.id && (
                                         <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-[#18181b] border border-black/10 dark:border-white/10 rounded-xl shadow-2xl z-50 p-1 animate-in fade-in zoom-in-95 duration-100 origin-top-right">
                                             <Link
                                                 href={`/core/playbooks/${doc.id}`}
                                                 className="px-3 py-2 text-xs font-medium text-zinc-600 dark:text-zinc-300 hover:text-black dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-lg flex items-center gap-2"
            ...
            {/* Pagination Controls */}
            {totalPages > 1 && (
                <div className="flex items-center justify-between mt-8 px-2 border-t border-black/5 dark:border-white/5 pt-4 mb-20">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="px-4 py-2 rounded-lg bg-zinc-100 dark:bg-zinc-900 border border-black/10 dark:border-white/10 text-xs font-bold hover:bg-zinc-200 dark:hover:bg-zinc-800 disabled:opacity-50 transition-colors text-black dark:text-white"
                        >
                            Previous
                        </button>
                        <button
             ...
             {showCreateModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/60 dark:bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="bg-white/90 dark:bg-[#09090b]/90 border border-black/10 dark:border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl scale-100 animate-in zoom-in-95 duration-200 backdrop-blur-xl ring-1 ring-black/10 dark:ring-white/10">
                        <div className="flex flex-col gap-6">
                            <div className="text-center">
                                <div className="w-12 h-12 rounded-full bg-zinc-200 dark:bg-zinc-800 mx-auto flex items-center justify-center mb-4 border border-black/5 dark:border-white/5">
                                    <FileText className="w-6 h-6 text-black dark:text-white"/>
                                </div>
                                <h3 className="text-xl font-bold text-black dark:text-white mb-2">New Playbook</h3>
                                <p className="text-sm text-zinc-500 dark:text-zinc-400">Enter a title to get started. By default, this will be visible only to <strong>Core</strong> members.</p>
                            </div>
                            ...
                            <input 
                                ...
                                className="w-full bg-white/50 dark:bg-black/50 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-black dark:text-white placeholder:text-zinc-400 dark:placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-zinc-300 dark:focus:ring-zinc-700 transition-all text-sm"
                            />

                            <div className="flex gap-3 w-full">
                                <button 
                                    onClick={() => setShowCreateModal(false)}
                                    className="flex-1 px-4 py-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-black/10 dark:border-white/10 text-sm font-semibold hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors"
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    className="flex-1 px-4 py-2.5 rounded-xl bg-black dark:bg-white text-white dark:text-black text-sm font-bold hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors disabled:opacity-50"
                                >
             ...
             {showDeleteConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/60 dark:bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="bg-white/90 dark:bg-[#09090b]/90 border border-black/10 dark:border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl scale-100 animate-in zoom-in-95 duration-200 backdrop-blur-xl ring-1 ring-red-500/20">
                        <div className="flex flex-col items-center text-center gap-4">
                            <div className="p-4 rounded-full bg-red-500/10 border border-red-500/20 text-red-500">
                                <Trash2 className="w-8 h-8"/>
                            </div>
                            <div>
                                <h3 className="text-xl font-bold text-black dark:text-white mb-2">Delete Playbook?</h3>
