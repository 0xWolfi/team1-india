            ...
            <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                    {members.length === 0 && !isLoading && (
                        <tr><td colSpan={4} className="p-12 text-center text-zinc-600">No operatives found in registry.</td></tr>
                    )}
                        ...
                        <tr key={member.id} className="hover:bg-white/[0.02] transition-colors group relative">
                            <td className="p-4 pl-6">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-zinc-800 to-black border border-white/10 flex items-center justify-center text-[10px] font-bold text-zinc-400 group-hover:border-white/20 transition-colors">
                                        {member.email.charAt(0).toUpperCase()}
                                    </div>
                                    <div>
                                        <div className="font-medium text-zinc-200 flex items-center gap-2 text-sm">
                                            {member.email}
                                            {isSelf && <span className="px-1.5 py-0.5 rounded-[4px] text-[9px] bg-white/10 text-zinc-300 border border-white/5 font-mono">YOU</span>}
                                        </div>
                                        <div className="text-[10px] text-zinc-600 font-mono mt-0.5">ID: {member.id.substring(0,8)}</div>
                                    </div>
                                </div>
                            </td>
                            <td className="p-4">
                                    ...
                                    {member.tags.length > 0 ? (
                                        <>
                                            {member.tags.slice(0, 2).map(tag => (
                                                <span key={tag} className="inline-flex items-center px-2 py-0.5 rounded-[4px] text-[10px] font-medium bg-zinc-900/50 text-zinc-400 border border-white/5">
                                                    {tag}
                                                </span>
                                            ))}
                                            {member.tags.length > 2 && (
                                                <span className="inline-flex items-center px-2 py-0.5 rounded-[4px] text-[10px] font-medium bg-zinc-800 text-zinc-500 border border-white/5">
                                                    +{member.tags.length - 2}
                                                </span>
                                            )}
                                        </>
                                    ) : <span className="text-zinc-700 text-[10px] italic">Unassigned</span>}
                                </div>
                            </td>
                            <td className="p-4">
                                ...
                                {(canManageUsers || canGrantAccess) && (
                                    <>
                                        ...
                                        {activeMenuId === member.id && (
                                            <div onClick={(e) => e.stopPropagation()} className="absolute right-10 top-6 w-56 bg-[#09090b] border border-white/10 rounded-xl shadow-[0_20px_40px_-5px_rgba(0,0,0,0.6)] z-30 py-1 overflow-hidden animate-in fade-in zoom-in-95 duration-100 ring-1 ring-white/5">
