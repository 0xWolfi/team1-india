...
export const DeleteMemberModal: React.FC<DeleteMemberModalProps> = ({ member, onClose, onConfirm, isSubmitting }) => {
    ...
    return (
        <div className="fixed inset-0 bg-white/80 dark:bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
            <div className="bg-white dark:bg-[#09090b] border border-red-500/20 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 ring-1 ring-red-500/10" onClick={e => e.stopPropagation()}>
                <div className="p-6 text-center">
                    ...
                    <h3 className="text-lg font-bold text-black dark:text-white mb-2">Terminate Operative?</h3>
                    <p className="text-zinc-500 dark:text-zinc-400 text-sm leading-relaxed mb-1">
                ...
                <div className="p-6 border-t border-black/5 dark:border-white/5 bg-black/[0.02] dark:bg-white/[0.02] flex gap-3">
                    <button
                        onClick={onClose}
                        className="flex-1 py-2.5 rounded-xl text-sm font-medium text-zinc-500 dark:text-zinc-400 hover:bg-black/5 dark:hover:bg-white/5 hover:text-black dark:hover:text-white transition-colors"
                    >
                        Cancel
                    </button>
                    <button 
