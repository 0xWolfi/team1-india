    ...
    return (
        <CoreWrapper>
            ...
            <div className="space-y-6">
                {/* Search & Tabs */}
                <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-zinc-900/50 p-1 rounded-xl border border-white/5">
                    ...
                    <div className="relative w-full md:w-64 mr-2">
                        ...
                        <input
                            ...
                            className="w-full bg-black/50 border border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm text-white focus:outline-none focus:border-white/20 transition-all placeholder:text-zinc-700"
                        />
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-20 border border-dashed border-white/10 rounded-2xl bg-white/[0.02]">
                        ...
                        <h3 className="text-zinc-400 font-medium">No assets found</h3>
                        <p className="text-zinc-600 text-xs mt-1">Try adjusting your filters or search terms.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {assets.map(asset => (
                            <div key={asset.id} className="group relative bg-zinc-900 border border-white/5 rounded-xl overflow-hidden hover:border-white/20 transition-all hover:translate-y-[-2px]">
                                {/* Preview Area */}
                                <div className="aspect-square bg-[url('/grid-pattern.svg')] bg-zinc-950 flex items-center justify-center p-8 relative border-b border-white/5">
                                    ...
                                    {asset.customFields?.format && (
                                        <div className="absolute top-3 right-3 px-2 py-1 bg-black/50 backdrop-blur-md border border-white/10 rounded text-[10px] font-mono text-zinc-400 uppercase">
                                            {asset.customFields.format}
                                        </div>
                                    )}
                                </div>
                                ...
                                {/* Info Area */}
                                <div className="p-4">
                                    <h3 className="text-sm font-bold text-white truncate mb-1">{asset.title}</h3>
                                    ...
                                    {/* Color Hex Display */}
                                    {asset.type === 'COLOR_PALETTE' && (
                                        <div className="mt-3 p-2 bg-black/30 rounded border border-white/5 flex justify-between items-center">
                                            <code className="text-xs text-zinc-300 font-mono">{asset.content}</code>
                                        </div>
                                    )}
                                </div>
                            </div>
