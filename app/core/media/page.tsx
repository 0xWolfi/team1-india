...
export default function MediaPage() {
    ...
    const STATUS_COLS = [
        { id: 'draft', label: 'Drafts', color: 'bg-zinc-800' },
    ...
    return (
        <CoreWrapper>
            ...
            <CorePageHeader 
                ...
                icon={<Film className="w-5 h-5 text-zinc-200" />} // Using Film icon
            >
                 <button 
                ...
                <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
                     ...
                     <div className="relative group w-full md:w-auto flex-1 max-w-md">
                        ...
                        <input 
                            ...
                            className="w-full bg-zinc-900/50 border border-white/5 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white focus:outline-none focus:ring-1 focus:ring-white/10 placeholder:text-zinc-600 font-medium transition-all hover:bg-zinc-900/80"
                        />
                    </div>
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto justify-end px-2">
                        <div className="flex bg-white/5 rounded-lg p-0.5 border border-white/5">
                            <button 
                                ...
                                className={`p-1.5 rounded-md transition-all ${viewMode === 'kanban' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
                            >
                    ...
                    <div className="flex gap-6 overflow-x-auto pb-8 snap-x h-[calc(100vh-340px)] scrollbar-hide">
                        {STATUS_COLS.map(col => {
                            ...
                            return (
                                <div 
                                    key={col.id} 
                                    className={`flex flex-col flex-1 min-w-[200px] snap-center h-full transition-colors rounded-2xl ${draggedItem && draggedItem.status !== col.id ? 'bg-white/[0.02] border border-white/5 border-dashed' : ''}`}
                                ...
                                >
                                    <div className="flex items-center justify-between mb-4 px-2">
                                        <div className="flex items-center gap-2.5">
                                            ...
                                            <h3 className="font-bold text-xs text-zinc-400 tracking-wider uppercase">{col.label}</h3>
                                        </div>
                                        <span className="text-[10px] font-mono text-zinc-500 bg-zinc-900/50 px-2 py-0.5 rounded-full border border-white/5">
                                            {colItems.length}
                                        </span>
                                    </div>
                                    
                                    <div className={`flex-1 rounded-2xl bg-[#121212]/30 border border-white/[0.02] p-2 space-y-3 overflow-y-auto hover:bg-[#121212]/50 transition-colors ${isColumnEmpty ? 'border-dashed border-white/5' : ''}`}>
                                        {colItems.map(item => (
                                            <div 
                                                key={item.id} 
                                                draggable
                                                ...
                                                className="group relative bg-[#18181b]/60 backdrop-blur-md border border-white/5 p-4 rounded-xl hover:border-white/20 transition-all cursor-grab active:cursor-grabbing overflow-hidden hover:translate-y-[-2px] hover:shadow-xl"
                                            >
                                                {/* Hover Glow */}
                                                ...
                                                <div className="relative z-10">
                                                    <div className="flex flex-wrap gap-1.5 mb-3">
                                                        {(Array.isArray(item.platform) ? item.platform : [item.platform || 'General']).slice(0, 3).map((p, idx) => (
                                                            <span key={idx} className="text-[9px] font-bold tracking-wider text-zinc-400 uppercase bg-zinc-900/80 px-2 py-1 rounded border border-white/5">
                                                                {p}
                                                            </span>
                                                        ))}
                                                    ...
                                                    <h4 className="font-semibold text-sm text-zinc-100 mb-2 leading-snug group-hover:text-white transition-colors">
                                                        {item.title || 'Untitled'}
                                                    </h4>
                                                    
                                                    {item.description && (
                                                        <p className="text-[11px] text-zinc-500 line-clamp-2 mb-4 leading-relaxed group-hover:text-zinc-400 transition-colors">
                                                            {item.description}
                                                        </p>
                                                    )}
                                                    ...
                                                    <div className="flex items-center justify-between pt-3 border-t border-white/5 mt-auto">
                                                        <div className="flex items-center gap-2">
                                                             <div className="w-5 h-5 rounded-full bg-gradient-to-br from-indigo-500/20 to-purple-500/20 flex items-center justify-center text-[9px] text-purple-300 font-bold border border-white/5">
                                        ...
                                        {isColumnEmpty && (
                                            <div className="h-full flex flex-col items-center justify-center text-zinc-700 py-12 opacity-60">
                                                <div className="w-10 h-10 rounded-full border border-white/5 flex items-center justify-center mb-2 bg-zinc-900/50">
