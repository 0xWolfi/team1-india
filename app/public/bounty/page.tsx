...
const glassClass =
    "bg-white/60 dark:bg-zinc-900/40 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.06]";
...
export default function PublicBountyPage() {
    ...
    if (status === "unauthenticated") {
        return (
            <main className="min-h-screen text-black dark:text-white flex flex-col">
                <FloatingNav />
                <div className="flex-1 flex items-center justify-center px-4 md:px-8">
                    <div className={cn("max-w-md w-full rounded-2xl p-8 flex flex-col items-center text-center", glassClass)}>
                        ...
                        <button
                            ...
                            className="px-6 py-3 bg-black text-white dark:bg-white dark:text-black font-semibold rounded-xl text-sm hover:opacity-90 transition-all flex items-center gap-2"
                        >
        ...
        return (
            <main className="min-h-screen text-black dark:text-white">
                ...
                <div className="pt-32 px-4 text-center">
                    <div className="animate-pulse text-zinc-500 dark:text-zinc-600">Loading...</div>
                </div>
            </main>
        );
    }
    ...
    return (
        <main className="min-h-screen flex flex-col text-black dark:text-white">
            ...
            <div className="flex-1 pt-20 md:pt-24 px-4 md:px-8 max-w-7xl w-full mx-auto pb-20">
                {/* Header */}
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
                    <div>
                        <Link href="/public" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-zinc-500 hover:text-black dark:hover:text-white transition-colors mb-4 group">
                    ...
                    <Link
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold text-zinc-600 dark:text-zinc-400 hover:text-black dark:hover:text-white bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 hover:border-black/20 dark:hover:border-white/20 transition-all shrink-0"
                    >
                ...
                {/* Bounty Grid */}
                {loading ? (
                    <div className="py-20 text-center text-zinc-500 dark:text-zinc-600 animate-pulse">Loading bounties...</div>
                ) : bounties.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {bounties.map((bounty) => {
                            ...
                            return (
                                <div key={bounty.id} className={cn("rounded-2xl p-5 transition-all duration-300 hover:border-black/10 dark:hover:border-white/10", glassClass)}>
                                    <div className="flex items-center justify-between mb-4">
                                        <span className={cn("px-2.5 py-1 rounded-lg text-[11px] font-semibold border", cfg.bg, cfg.color, cfg.border)}>
                                            {cfg.label}
                                        </span>
                                        <span className="text-[11px] text-zinc-500 dark:text-zinc-600 font-medium">{frequencyLabel[bounty.frequency] || bounty.frequency}</span>
                                    </div>
                                    <div className="flex items-start gap-3 mb-3">
                                        ...
                                        <div className="min-w-0">
                                            <h3 className="font-semibold text-black dark:text-white text-sm leading-snug mb-1">{bounty.title}</h3>
                                            {bounty.description && <p className="text-xs text-zinc-500 dark:text-zinc-600 line-clamp-2">{bounty.description}</p>}
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-black/5 dark:border-white/5">
                                        <div className="flex items-center gap-1.5">
                                            <Zap className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400"/>
                                            <span className="text-sm font-bold text-amber-500 dark:text-amber-400">{bounty.xpReward} XP</span>
                                        </div>
                                        {isPublicUser ? (
                                            hasSubmission ? (
                                                <span className={cn(
                                                    "px-3 py-1.5 rounded-xl text-xs font-semibold border",
                                                    submissionStatus === 'approved' ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20" :
                                                    submissionStatus === 'rejected' ? "bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20" :
                                                ...
                                                <button
                                                    ...
                                                    className="px-4 py-2 rounded-xl text-xs font-semibold bg-black text-white dark:bg-white dark:text-black hover:opacity-90 transition-all"
                                                >
                                                    Submit
                                                </button>
                                            )
                                        ) : (
                                            <span className="text-[11px] text-zinc-500 dark:text-zinc-600">Public users only</span>
                                        )}
                                    </div>
                                </div>
                            );
                    ...
                    <div className={cn("w-full py-16 rounded-2xl flex flex-col items-center justify-center border-dashed", glassClass)}>
                        <Zap className="w-8 h-8 text-zinc-400 dark:text-zinc-700 mb-3"/>
                        <p className="text-zinc-500 dark:text-zinc-600 font-medium text-sm">No active bounties right now</p>
                        <p className="text-xs text-zinc-400 dark:text-zinc-700 mt-1">Check back soon!</p>
                    </div>
                )}
                ...
                {isPublicUser && submissions.length > 0 && (
                    <div className="mt-12">
                        <h2 className="text-lg font-bold text-black dark:text-white mb-4">Your Submissions</h2>
                                ...
                                return (
                                    <div key={sub.id} className={cn("rounded-xl p-4 flex items-center gap-4", glassClass)}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <a href={sub.proofUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-sky-600 dark:text-sky-400 hover:underline truncate block">{sub.proofUrl}</a>
                                        </div>
                                        <div className="flex items-center gap-3 shrink-0">
                                            {sub.status === 'approved' && sub.xpAwarded && (
                                                <span className="text-xs font-bold text-amber-600 dark:text-amber-400">+{sub.xpAwarded} XP</span>
                                            )}
            ...
            {/* Submit Modal */}
            {submitModal && (
                <div className="fixed inset-0 bg-black/40 dark:bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setSubmitModal(null)}>
                    <div className={cn("w-full max-w-lg rounded-2xl p-6 animate-in zoom-in-95 duration-200", "bg-white dark:bg-zinc-900/90 backdrop-blur-2xl border border-black/10 dark:border-white/10")} onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-lg font-bold text-black dark:text-white">Submit Proof</h3>
                            <button type="button" onClick={() => setSubmitModal(null)} className="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 text-zinc-500 dark:text-zinc-400">
                                <X className="w-4 h-4"/>
                            </button>
                        </div>
                        <div className={cn("p-3 rounded-xl mb-5 flex items-center gap-3", glassClass)}>
                            ...
                            <div>
                                <p className="text-sm font-medium text-black dark:text-white">{submitModal.title}</p>
                                <p className="text-xs text-amber-600 dark:text-amber-400 font-semibold">{submitModal.xpReward} XP</p>
                            </div>
                        </div>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Proof URL *</label>
                                <input type="url" value={proofUrl} onChange={e => setProofUrl(e.target.value)} required
                                    placeholder="https://twitter.com/... or blog link"
                                    className="w-full bg-zinc-100 dark:bg-zinc-800/50 border border-black/10 dark:border-white/10 rounded-xl px-4 py-2.5 text-black dark:text-white text-sm placeholder:text-zinc-400 focus:outline-none focus:border-black/20 dark:focus:border-white/20" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Note (optional)</label>
                                <textarea value={proofNote} onChange={e => setProofNote(e.target.value)}
                                    placeholder="Any context about your submission..." rows={2}
                                    className="w-full bg-zinc-100 dark:bg-zinc-800/50 border border-black/10 dark:border-white/10 rounded-xl px-4 py-2.5 text-black dark:text-white text-sm placeholder:text-zinc-400 focus:outline-none focus:border-black/20 dark:focus:border-white/20 resize-none" />
                            </div>
                            <div className="flex justify-end gap-3 pt-2">
                                <button type="button" onClick={() => setSubmitModal(null)} className="px-4 py-2 rounded-xl text-sm text-zinc-600 dark:text-zinc-400 hover:bg-black/5 dark:hover:bg-white/5">Cancel</button>
                                <button type="submit" disabled={submitting || !proofUrl}
                                    className="px-5 py-2 rounded-xl text-sm font-semibold bg-black text-white dark:bg-white dark:text-black hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed">
