  ...
  if (loading) return (
      <MemberWrapper>
        <div className="flex items-center justify-center h-[50vh] text-white">
            <MotionIcon name="Loader2" className="animate-spin w-8 h-8 text-indigo-500" animation="spin" />
        </div>
      </MemberWrapper>
  );
    ...
    <MemberWrapper>
        <div className="max-w-6xl mx-auto pb-20">
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                {/* Experiment Card */}
                <div className="bg-zinc-900/60 backdrop-blur-2xl border border-white/10 rounded-2xl overflow-hidden relative group shadow-2xl">
                    <div className="p-8">
                        <h1 className="text-3xl md:text-4xl font-bold mb-8 leading-tight text-white">{experiment.title}</h1>

                        <div className="flex items-start justify-between gap-4 mb-8 pb-8 border-b border-white/10">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-zinc-400 uppercase tracking-widest">{experiment.createdBy?.email}</div>
                                </div>
                            </div>
                        ...
                        <div className="prose prose-invert prose-p:text-zinc-200 prose-headings:text-white max-w-none">
                ...
                {isSuperAdmin && (
                    <div className="bg-zinc-900/60 backdrop-blur-2xl border border-white/10 rounded-2xl p-6 shadow-2xl">
                        ...
                        <div className="flex flex-wrap gap-3">
                            ...
                            {experiment.stage === 'PROPOSED' && (
                                ...
                                >
                                    {actionLoading ? <MotionIcon name="Loader2" className="w-3 h-3 animate-spin" animation="spin" /> : <MotionIcon name="MessageSquare" className="w-3 h-3" />}
                                    Open for Discussion
                                </button>
                            )}
                            ...
                            {experiment.stage !== 'APPROVED' && experiment.stage !== 'REJECTED' && (
                                <>
                                    ...
                                    >
                                        {actionLoading ? <MotionIcon name="Loader2" className="w-3 h-3 animate-spin" animation="spin" /> : <MotionIcon name="CheckCircle" className="w-3 h-3" />}
                                        Approve
                                    </button>
                                    <button 
                                        onClick={async () => {
                                    ...
                                    >
                                        {actionLoading ? <MotionIcon name="Loader2" className="w-3 h-3 animate-spin" animation="spin" /> : <MotionIcon name="XCircle" className="w-3 h-3" />}
                                        Reject
                                    </button>
                                </>
                            )}
                ...
                {/* Discussion */}
                <div className="bg-zinc-900/60 backdrop-blur-2xl border border-white/10 rounded-2xl p-8 relative shadow-2xl">
                     {/* Connection Line */}
                     <div className="absolute -top-6 left-12 w-0.5 h-6 bg-gradient-to-b from-white/10 to-white/5" />
                            ...
                            <div key={comment.id} className="relative pl-14 group">
                                ...
                                <div className="absolute left-0 top-0">
                                     {comment.author?.image ? (
                                        // eslint-disable-next-line @next/next/no-img-element
                                        <img src={comment.author.image} alt="User" className="w-10 h-10 rounded-full border-2 border-zinc-900 relative z-10" />
                                    ) : (
                                        <div className="w-10 h-10 rounded-full bg-zinc-800 border-2 border-zinc-900 flex items-center justify-center text-xs relative z-10">
                    ...
                    {experiment.stage === 'DISCUSSION' && (
                        <form onSubmit={handlePostComment} className="relative mt-8">
                            <div className="relative group">
                                ...
                                <textarea
                                    className="relative block w-full bg-black/40 border border-white/10 rounded-xl p-4 pr-14 text-sm text-white focus:outline-none focus:border-white/20 transition-all placeholder:text-zinc-500 min-h-[120px] resize-none"
                                ...
                                >
                                    {submittingComment ? <MotionIcon name="Loader2" className="w-4 h-4 animate-spin" animation="spin" /> : <MotionIcon name="Send" className="w-4 h-4" />}
                                </button>
                            </div>
                        </form>
                    )}
