...
export default function NewMemberExperimentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/member/experiments");
      router.refresh();
    } catch (error) {
      console.error(error);
  ...
  return (
    <MemberWrapper>
        <CorePageHeader
            ...
            backLink="/member/experiments"
        />

        <div className="max-w-3xl mx-auto">
            <div className="bg-[#121212]/80 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-8 shadow-2xl">
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        ...
                        <input
                            required
                            type="text"
                            className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all placeholder:text-zinc-600 font-bold"
                    ...
                    <div>
                        ...
                        <div className="relative group">
                            ...
                            <textarea
                                required
                                rows={8}
                                className="relative w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500/50 transition-all placeholder:text-zinc-600 resize-none text-sm leading-relaxed"
                    ...
                    <div className="pt-6 flex justify-end">
                        ...
                        >
                            {loading ? <MotionIcon name="Loader2" className="w-4 h-4 animate-spin" animation="spin" /> : <MotionIcon name="Save" className="w-4 h-4" />}
                            Submit Proposal
                        </button>
                    </div>
                </form>
