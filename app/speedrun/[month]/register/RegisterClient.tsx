...
export default function RegisterClient({ runSlug }: { runSlug: string }) {
  ...
  const registerUrl = `/speedrun/${encodeURIComponent(runSlug)}/register`;
  const statusUrl = `/speedrun/${encodeURIComponent(runSlug)}/registration`;

  useEffect(() => {
    if (status === "unauthenticated") {
      try {
        sessionStorage.setItem("postLoginRedirect", registerUrl);
      } catch {}
  ...
  // If already registered, send them to the status page. If the run isn't
  // accepting registrations (closed, future, cancelled), bounce them to the
  // run details page so they aren't filling out a form that the API will
  // reject on submit.
  useEffect(() => {
    ...
    fetch(`/api/speedrun/runs/${encodeURIComponent(runSlug)}/my-registration`)
      ...
      .then((data) => {
        ...
        if (data.run && data.run.status !== "registration_open") {
          router.replace(`/speedrun/${encodeURIComponent(runSlug)}`);
        }
      })
      .catch(() => {});
  }, [status, router, runSlug, statusUrl]);

  // Debounced team-code lookup, scoped to this run.
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/speedrun/runs/${encodeURIComponent(runSlug)}/teams/lookup?code=${encodeURIComponent(code)}`
        );
    ...
    return () => clearTimeout(t);
  }, [form.teamCode, form.teamMode, runSlug]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/speedrun/runs/${encodeURIComponent(runSlug)}/register`, {
        method: "POST",
    ...
    return (
      <main className="relative min-h-[100svh] bg-[var(--background)] text-black dark:text-white">
        ...
        <PublicLoginModal
          ...
          onClose={() => {
            setLoginModalOpen(false);
            router.push(`/speedrun/${encodeURIComponent(runSlug)}`);
          }}
        />
        <div className="pt-32 px-6 text-center">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-red-500 mb-3">Sign in required</p>
          <h1
          ...
          <p className="text-sm sm:text-base text-zinc-600 dark:text-zinc-400 max-w-md mx-auto mb-8">
            Sign in with Google to register for this Speedrun.
          </p>
          <button
      ...
      <main className="relative min-h-[100svh] bg-[var(--background)] text-black dark:text-white overflow-x-hidden">
        ...
        <section className="relative pt-28 sm:pt-36 pb-24">
          <div className="relative max-w-3xl mx-auto px-4 sm:px-6 md:px-8 text-center">
            ...
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-red-500 mb-3">You’re In</p>
            <h1
              className="font-black italic tracking-tighter leading-[0.9] text-black dark:text-white mb-4"
            ...
            {success.teamCode && (
              <div className="relative max-w-md mx-auto rounded-2xl border border-red-500/30 bg-red-500/5 p-5 sm:p-6 mb-8">
                ...
                <p className="text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
                  Share this Team1 ID with your teammate — they enter it on the registration form to join.
                </p>
              </div>
            )}
    ...
    <main className="relative min-h-[100svh] bg-[var(--background)] text-black dark:text-white overflow-x-hidden">
      ...
      <section className="relative pt-28 sm:pt-36 pb-8 sm:pb-12 overflow-hidden">
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 md:px-8">
          ...
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-red-500 mb-3">Register</p>
          <h1
      ...
      <form onSubmit={onSubmit} className="relative max-w-3xl mx-auto px-4 sm:px-6 md:px-8 pb-24 space-y-6 sm:space-y-8">
        <FormCard title="About You" subtitle="The basics — who you are and where to reach you.">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="City">
              <Input value={form.city} onChange={(e) => update("city", e.target.value)} placeholder="Bengaluru" />
            </Field>
          ...
          <label className="flex items-start gap-3 cursor-pointer mt-4">
            ...
            <span className="text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
              Show my X and GitHub on the public team page (your email is always private).
            </span>
          </label>
        </FormCard>
        ...
        <FormCard title="Your Build Profile" subtitle="What you do and how deep you go.">
          <Field label="Primary role *">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {ROLE_OPTIONS.map((r) => (
                <PillButton key={r} active={form.primaryRole === r} onClick={() => update("primaryRole", r)}>
                  {r}
                </PillButton>
              ))}
            </div>
        ...
        <FormCard title="Team" subtitle="Solo, lead a duo, or join an existing team. (Max 2 builders per team.)">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            ...
            <TeamModeOption
              ...
              title="Create Team"
              desc="Get a Team1 ID, invite a partner"
          ...
          {form.teamMode === "create" && (
            <p className="text-xs text-zinc-500 leading-relaxed">
              You’ll get a unique Team1 ID after submitting — share it with your partner so they can join.
            </p>
          )}

          {form.teamMode === "join" && (
            <Field label="Team1 ID *">
              ...
              {teamPreview && (
                <div className="mt-3 p-3 rounded-lg bg-green-500/5 border border-green-500/20 flex items-start gap-2">
                  ...
                  <div className="text-xs">
                    <div className="font-bold text-black dark:text-white">Team {teamPreview.team1Id}</div>
                    <div className="text-zinc-500">
                      {teamPreview.memberCount} member
          ...
          {form.teamMode === "solo" && (
            <p className="text-xs text-zinc-500 leading-relaxed">
              ...
              You can switch to a team anytime before submissions close.
            </p>
          )}
        </FormCard>
        ...
        <div className="rounded-2xl border border-red-500/15 bg-white/40 dark:bg-zinc-950/40 p-5 sm:p-6">
          <label className="flex items-start gap-3 cursor-pointer mb-5">
            ...
            <span className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">
              I agree to the Speedrun code of conduct and understand my project details may be shared with sponsors for judging and amplification.
            </span>
          </label>
