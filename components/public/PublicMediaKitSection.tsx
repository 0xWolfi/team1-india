...
import { Download, ExternalLink, Hash, Image as ImageIcon, Video } from 'lucide-react';
        ...
        <section className="space-y-8">
             <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    ...
                    <p className="text-zinc-400 text-sm">Official brand assets, logos, and media content.</p>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {mediaItems.map((item) => (
                    <div key={item.id} className="bg-zinc-900/50 border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-all group flex flex-col h-full">
                        <div className="flex items-start justify-between mb-4">
                            <div className="flex flex-wrap gap-2">
                                {item.platform && item.platform.length > 0 ? item.platform.map((p, i) => (
                                    <span key={i} className="text-[10px] font-bold uppercase tracking-wider bg-white/5 text-zinc-400 px-2 py-1 rounded-full border border-white/5">
                                        {p}
                                    </span>
                                )) : (
                                    <span className="text-[10px] font-bold uppercase tracking-wider bg-white/5 text-zinc-400 px-2 py-1 rounded-full border border-white/5">
                                        Asset
                                    </span>
                                )}
                            </div>
                        ...
                        <div className="mt-auto pt-4 border-t border-white/5 flex flex-col gap-2">
                            {item.links && item.links.length > 0 ? (
                                ...
                                <div className="text-xs text-zinc-600 italic px-2">No links attached</div>
                            )}
                        </div>
                    </div>
                ))}
