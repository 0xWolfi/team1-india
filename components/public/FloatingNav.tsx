    ...
    return (
        <>
            <div className={cn(
                ...
                // Base (Mobile): Full width, top 0, transparent, safe area
                    ...
                    <div className="flex items-center gap-2 pl-1">
                        {session?.user ? (
                            <button
                                ...
                                className="flex items-center gap-2 rounded-xl text-zinc-500 hover:text-black dark:text-zinc-400 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 transition-all md:p-1 md:pl-2 md:pr-2"
                            >
                                <div className="w-8 h-8 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-black dark:text-white text-xs font-bold ring-1 ring-black/10 dark:ring-white/10 overflow-hidden relative">
                                    {session.user.image ? (
                                        <Image
                            ...
                            <button
                                ...
                                className="w-8 h-8 flex items-center justify-center text-zinc-500 hover:text-black dark:text-zinc-400 dark:hover:text-white transition-colors md:p-2 md:w-auto md:h-auto"
                                title="Member Access"
                            >
            ...
            {showUserMenu && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="relative w-full max-w-sm bg-white/90 dark:bg-zinc-900/80 backdrop-blur-xl border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl p-6 animate-in zoom-in-95 duration-200">
                         <button
                            onClick={() => setShowUserMenu(false)}
                            className="absolute top-4 right-4 text-zinc-400 hover:text-black dark:text-zinc-500 dark:hover:text-white transition-colors"
                        >
                            <X className="w-5 h-5"/>
                        </button>

                        <div className="flex items-center gap-4 mb-6">
                            <div className="w-16 h-16 rounded-full bg-indigo-500 flex items-center justify-center text-white text-2xl font-bold border-2 border-zinc-200 dark:border-zinc-800 shadow-xl overflow-hidden relative">
                                {session?.user?.image ? (
                                    <Image
                                        src={session.user.image}
                                        alt={session.user.name || "User"}
                            ...
                            <div>
                                <div className="font-bold text-black dark:text-white text-lg">{session?.user?.name || "User"}</div>
                         ...
                         <div className="space-y-3">
                            ...
                            {((session?.user as any)?.role === 'CORE' || (session?.user as any)?.role === 'MEMBER') && (
                                <Link
                                    ...
                                    className="group w-full py-3 bg-black dark:bg-white text-white dark:text-black font-bold rounded-xl hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-all flex items-center justify-center gap-2"
                                >
                            ...
                            <button
                                onClick={() => signOut()}
                                className="group w-full py-3 bg-black/5 dark:bg-white/5 text-zinc-500 dark:text-zinc-400 font-bold rounded-xl hover:bg-red-500/10 hover:text-red-500 border border-black/5 dark:border-white/5 hover:border-red-500/20 transition-all flex items-center justify-center gap-2"
                            >
            ...
            {/* Mobile Bottom Dock */}
            <div className="md:hidden fixed bottom-[calc(env(safe-area-inset-bottom,0px)+1.5rem)] left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-sm bg-white/80 dark:bg-black/80 backdrop-blur-2xl border border-black/10 dark:border-white/10 rounded-2xl px-3 py-2.5 flex items-center justify-between shadow-2xl">
                {navItems.map((item) => {
