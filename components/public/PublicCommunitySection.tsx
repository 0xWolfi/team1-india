            ...
            {partners.length > 0 && (
                <section>
                    <div className="flex items-center gap-3 mb-8">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-black dark:text-white">Partners & Ecosystem</h2>
                            <p className="text-zinc-600 dark:text-zinc-400 text-sm">Organizations supporting our mission.</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
                        {partners.map(partner => (
                            <div key={partner.id} className="bg-zinc-100/50 dark:bg-zinc-900/50 border border-black/5 dark:border-white/5 rounded-2xl p-6 hover:border-black/10 dark:hover:border-white/10 transition-all flex flex-col items-center text-center group">
                                {partner.logo ? (
                                    <Image src={partner.logo} alt={partner.name} width={64} height={64} className="w-16 h-16 rounded-full object-cover bg-white dark:bg-black mb-4 border border-black/5 dark:border-white/5" />
                                ) : (
                                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-zinc-200 to-zinc-100 dark:from-zinc-800 dark:to-zinc-900 border border-black/5 dark:border-white/5 flex items-center justify-center text-xl font-bold text-zinc-500 uppercase mb-4">
                                        {partner.name.substring(0, 2)}
                                    </div>
                                )}

                                <h3 className="font-bold text-black dark:text-white mb-1 group-hover:text-emerald-500 dark:group-hover:text-emerald-400 transition-colors">{partner.name}</h3>
                                {partner.type && (
                                    <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 bg-black/5 dark:bg-white/5 px-2 py-1 rounded-full mb-3">
                                        {partner.type}
                                    </span>
                                )}
